...
# Define RNN Model
# ==============================================================
def RNN_simple(optuna_trial, tuning_params):
    """
    Generate sequential network model with optuna optimization.
...
def validate_one_epoch(model, val_loader, loss_function, device):
    ...
    avg_loss = avg_loss / len(val_loader.dataset)
    return avg_loss
...
def train_val_loop(model, training_params, tuning_params, X_train, y_train, X_val, y_val, device):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch >= 20 and epochs_improvement >= early_stop_patience:
            ...
            model = best_model
            return predict(model, val_loader, device), early_stopping_point
    
    return predict(best_model, val_loader, device), early_stopping_point

# ==============================================================
# Define objective function for tuning hyperparameters
# ==============================================================
def objective(trial, X, y, training_params_dict, device):
    ...
    # create model
    try:
        model = RNN_simple(trial, tuning_params=tuning_params_dict)       
    except Exception as err:
        print('Trial failed. Error in model creation, {}'.format(err))
        raise optuna.exceptions.TrialPruned()
    
    # load model to the device in case on gpu
    model.to(device)
...
def tuning_RNN(datapath, X, y, data_variants, training_params_dict, device):
    ...
    study.optimize(lambda trial: objective(trial, X, y, training_params_dict, device), n_trials=num_trials)
    ...
    fig_optim_history.write_image("./optimhisto_rnn_pheno" + pheno + minmax + standard + pcafitting + ".pdf")
    fig_inter_values.write_image("./intervalue_rnn_pheno" + pheno + minmax + standard + pcafitting + ".pdf")
