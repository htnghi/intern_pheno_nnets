...
def RNN_simple(tuning_params):
    """
    RNN model with hyperparameter tuning by optuna optimization.

    :param n_layers: the number of stacks in each cells
    ...
    if tuning_params['n_layers'] > 1:
        ...
        # if the RNN has only one layer, the dropout value is ignored
        layers.append(RNN(input_size=n_feature, hidden_size=tuning_params['hidden_size'],
...
def train_val_loop(model, training_params, tuning_params, X_train, y_train, X_val, y_val, device):
    ...
    # squeeze y to get suitable y dims for training RNN
...
def objective(trial, X, y, data_variants, training_params_dict, avg_stop_epochs, device):
    ...
    # create model
    try:
        model = RNN_simple(tuning_params=tuning_params_dict).to(device)     
...
def tuning_RNN(datapath, X, y, data_variants, training_params_dict, device):
    ...
    study = optuna.create_study(
        study_name='rnn_'+'mseloss_'+'data'+pheno+minmax+standard+pcafitting,
        direction ="minimize",
...
def evaluate_result_RNN(datapath, X_train, y_train, X_test, y_test, best_params, data_variants, device):
    ...
    # squeeze y for training RNN to tensor
    ...
    print('--------------------------------------------------------------')
    print('Test RNN results: avg_loss={:.4f}, avg_expvar={:.4f}, avg_r2score={:.4f}, avg_mae={:.4f}'.format(test_mse, test_exp_variance, test_r2, test_mae))
    print('--------------------------------------------------------------')
