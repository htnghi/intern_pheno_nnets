...
# Define MLP Model
# ==============================================================
def MLP(in_features, tuning_params):
    """
    MLP model with hyperparameter tuning by optuna optimization.

    :param n_layers: number of hidden layers
    :param outfactor: the ratio of the number of nodes to the number of input features
...
def train_val_loop(model, training_params, tuning_params, X_train, y_train, X_val, y_val, device):
    ...
    # squeeze y to get suitable y dims for training MLP
...
def tuning_MLP(datapath, X, y, data_variants, training_params_dict, device):
    ...
    study = optuna.create_study(
        study_name='mlp_'+'mseloss_'+'data'+pheno+minmax+standard+pcafitting,
        direction="minimize",
