...
# Define CNN Model
# ==============================================================
def CNN1D(num_features, tuned_params):
    """
    ...
    act_func1 = get_activation_func(tuned_params['activation1'])
    act_func2 = get_activation_func(tuned_params['activation2'])

    for i in range(tuned_params['n_layers']):

        # print('DBG: conv layer {:2d}, in={:4d}, out={:4d}'.format(i, in_filters, out_filters))
        stride = max(1, int(tuned_params['kernel_size'] * tuned_params['stride_percentage']))

        layers.append(Conv1d(in_filters, out_filters, kernel_size=tuned_params['kernel_size'], stride=stride))
    ...
    layers.append(MaxPool1d(tuned_params['kernel_size']))
    ...
    out_linear_features = int(in_linear_features * tuned_params['factor_out_linear_features'])
...
def train_loop(X_train, y_train, hyperparameters):
    ...
    tensor_X_train = torch.swapaxes(tensor_X_train, 1, 2)
    ...
    model = CNN1D(num_features=n_inputs, tuned_params=hyperparameters)
...
def run_train_CNN(datapath, X_train, y_train, X_test, y_test, hyperparameters, data_variants):
    ...
    figurename = datapath + '/test_cnn_pheno' + pheno + minmax + standard + pcafitting + '.svg'
